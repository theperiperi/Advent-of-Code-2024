...
def reorder_update(update, graph, in_degree):
    # Filter the graph for only pages in this update
    sub_graph = defaultdict(list)
    sub_in_degree = defaultdict(int)
...
def part2(input_text):
    ...
    for update in updates:
        if not is_valid_update(update, graph, in_degree):
            # Reorder the update
            corrected_update = reorder_update(update, graph, in_degree)
            # Find the middle page of the corrected update
            total += find_middle_page(corrected_update)
    return total

filename = r'day05/input.txt'
