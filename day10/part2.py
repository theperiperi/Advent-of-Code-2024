...
def part2(puzzle_input):
    ...
    # Calculate total paths by summing paths from all starting positions
    # Using zip to combine results and map to sum the second elements (path counts)
    _, p2 = map(sum, zip(*(get_score_and_rating(grid, p) for p in trailheads)))
    return p2
